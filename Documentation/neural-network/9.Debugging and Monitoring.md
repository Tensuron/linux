...
## [2025 Update]
### DebugFS
- Statistics are exposed via DebugFS using `debugfs_create_u64` for atomic64_t fields:
  - predictions_made
  - cache_hits
  - cache_misses
- These are visible in `/sys/kernel/debug/neural/`

### Stats Functions
- Use `neural_network_print_stats(nn)` to print stats to the kernel log.
- Stats include predictions made, cache hits/misses, errors encountered, last error string, and timing info.
